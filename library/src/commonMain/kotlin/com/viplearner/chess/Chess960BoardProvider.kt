...
interface Chess960BoardProvider : BoardProvider {
    fun fromPosition(position: Int): Board {
        return fromFEN(positionToFEN(position))
    }

    fun random960(): Board {
        return fromPosition(Random.nextInt(960))
    }
    ...
    fun initialCastlingByPiecesSetup(pieces: String): String {
        return (0 until 8)
            .asSequence()
            .filter { i -> pieces[i] == 'r' }
            .map { i -> ('a' + i).toString() }
    ...
    fun positionToFEN(position: Int): String {
        val pieces = when (position) {
            0 -> "bbqnnrkr"
            1 -> "bqnbnrkr"
            2 -> "bqnnrbkr"
            3 -> "bqnnrkrb"
