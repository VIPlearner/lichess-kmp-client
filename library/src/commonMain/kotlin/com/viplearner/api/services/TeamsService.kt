...
class TeamsService(
     ...
     */
    suspend fun teamSearch(text: String? = null, page: Int? = null): Result<TeamPaginatorJson> {
        return try {
            val queryParams = mapOf(
                "text" to text,
                "page" to page
     ...
     */
    suspend fun teamIdJoin(teamId: String, message: String? = null, password: String? = null): Result<Ok> {
        return try {
            val formBody = mapOf(
                "message" to message,
     ...
     */
    suspend fun teamRequests(teamId: String, declined: Boolean? = null): Result<List<TeamRequestWithUser>> {
        return try {
            val queryParams = mapOf(
     ...
     */
    suspend fun teamRequestAccept(teamId: String, userId: String): Result<Ok> {
        return try {
     ...
     */
    suspend fun teamRequestDecline(teamId: String, userId: String): Result<Ok> {
        return try {
     ...
     * - <https://lichess.org/team>
     */
    suspend fun teamIdKickUserId(teamId: String, userId: String): Result<Ok> {
        return try {
     ...
     */
    suspend fun teamIdPmAll(teamId: String, message: String? = null): Result<Ok> {
        return try {
            val formBody = mapOf(
                "message" to message
            ).filterValues { it != null }
